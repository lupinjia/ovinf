#include "ovinf/ovinf_humanoid_walk.h"

namespace ovinf {

HumanoidWalkPolicy::~HumanoidWalkPolicy() {
  exiting_.store(true);
...
HumanoidWalkPolicy::HumanoidWalkPolicy(const YAML::Node &config) : BasePolicy(config) {
  // Read config
  ...
  worker_thread_ = std::thread(&HumanoidWalkPolicy::WorkerThread, this);
}
...
std::optional<HumanoidWalkPolicy::VectorT> HumanoidWalkPolicy::GetResult(
...
void HumanoidWalkPolicy::PrintInfo() {
...
void HumanoidWalkPolicy::WorkerThread() {
  if (realtime_) {
...
void HumanoidWalkPolicy::CreateLog(YAML::Node const &config) {
...
void HumanoidWalkPolicy::WriteLog(
    ProprioceptiveObservation<float> const &obs_pack) {
