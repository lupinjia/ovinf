#include "ovinf/ovinf_humanoid_walk_clock.h"

namespace ovinf {

HumanoidWalkClockPolicy::~HumanoidWalkClockPolicy() {
  exiting_.store(true);
...
HumanoidWalkClockPolicy::HumanoidWalkClockPolicy(const YAML::Node &config) : BasePolicy(config) {
  // Read config
  ...
  worker_thread_ = std::thread(&HumanoidWalkClockPolicy::WorkerThread, this);
}

bool HumanoidWalkClockPolicy::WarmUp(ProprioceptiveObservation<float> const &obs_pack) {
...
bool HumanoidWalkClockPolicy::InferUnsync(
  ...
  {
    command_scaled(0) = obs_pack.command(0) * 1.5;
...
std::optional<HumanoidWalkClockPolicy::VectorT> HumanoidWalkClockPolicy::GetResult(
...
void HumanoidWalkClockPolicy::PrintInfo() {
...
void HumanoidWalkClockPolicy::WorkerThread() {
  if (realtime_) {
...
void HumanoidWalkClockPolicy::CreateLog(YAML::Node const &config) {
...
void HumanoidWalkClockPolicy::WriteLog(
    ProprioceptiveObservation<float> const &obs_pack) {
